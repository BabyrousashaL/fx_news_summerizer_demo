...
#全局变量声明
"""文件参数"""
path = r'E:\AI Lab\数据封禁线索\线索归档\Fox_Videos'
"""时间参数"""
days = 2
"""大模型参数"""
...
def get_news_list(earliest_date):
    try:
        """获取api提供的新闻数据"""
        all_news_articles = []
        url = "https://api.foxnews.com/v3/videos?from=0&size=100"
...
def get_row_video_subtitles(row):
    id = getattr(row,'id')
    player_url = f"https://api.foxnews.com/v3/video-player/{id}"
    player_response = get_page_content(player_url)
    vtt_url = (
